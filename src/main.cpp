...
#if defined(NDEBUG)
# error "Kudos-beta cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kudos-beta: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kudos-beta Signed Message:\n";
...
{
    // Kudos-beta: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Kudos-beta: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Kudos-beta: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Kudos-beta: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kudos-beta: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kudos-beta: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kudos-beta: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kudos-beta: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Kudos-betaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Kudos-betaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static Kudos-betaMiner(CWallet *pwallet)
{
    printf("Kudos-betaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kudos-beta-miner");
    ...
    try { loop {
        ...
        printf("Running Kudos-betaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Kudos-betaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Kudos-betaMiner, pwallet));
}
